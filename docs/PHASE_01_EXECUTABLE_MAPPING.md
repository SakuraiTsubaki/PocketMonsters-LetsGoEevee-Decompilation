...
Establish a reproducible executable baseline for the exact local Pokémon: Let's Go, Eevee! target before function naming or source reconstruction begins.
...
python tools/inventory_extracted_tree.py /path/to/extracted-game \
  --project "Pokemon: Let's Go, Eevee!" --platform "Nintendo Switch" \
...
Record Module ID / build ID, `.text`, `.rodata`, `.data` file and memory ranges, stored/mapped sizes, compression and hash flags, BSS, and dynamic string/symbol table metadata.
