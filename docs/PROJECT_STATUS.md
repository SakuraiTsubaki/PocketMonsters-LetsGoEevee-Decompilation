...
- [ ] Inventory the first verified Let's Go Eevee target
...
Run `tools/inventory_target.py` on the first local Let's Go Eevee target or extracted tree, register exact identity, then begin ExeFS/RomFS/module mapping. Retail bytes and keys remain local.
